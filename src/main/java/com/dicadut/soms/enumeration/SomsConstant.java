package com.dicadut.soms.enumeration;

/**
 * @author fan_jennifer
 * @version 1.0
 * @description 常量类用来封装简单常量
 * @date 2022-02-24 21:16
 */
public final class SomsConstant {
    public static final String TASK = "t_task";

    //    diseaseDetailDTO.setFeatureFields(map.getOrDefault(1, new ArrayList<>()));
//        diseaseDetailDTO.setFeaturePopups(map.getOrDefault(2, new ArrayList<>()));
//        diseaseDetailDTO.setFeatureRadios(map.getOrDefault(3, new ArrayList<>()));
//        diseaseDetailDTO.setDiseasePictures(map.getOrDefault(4, new ArrayList<>()));
//        diseaseDetailDTO.setDiseaseVoices(map.getOrDefault(5, new ArrayList<>()));
//        diseaseDetailDTO.setDiseaseTexts(map.getOrDefault(6, new ArrayList<>()));
    public static final String FEATURE_FIELD = "1";
    public static final String FEATURE_POPUP = "2";
    public static final String FEATURE_RADIO = "3";
    public static final String DISEASE_PICTURE = "4";
    public static final String DISEASE_VIDEO = "5";
    public static final String DISEASE_VOICE = "6";
    public static final String DISEASE_TEXT = "7";
    public static final String DISEASE_REVIEW_OPINION = "8";

    //dictionary
    public static final String TYPE = "type";
    //task_bridge_component
    public static final String TASK_ID = "task_id";

}
