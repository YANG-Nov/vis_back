...
#JAVA_OPTS="-Xms2G -Xmx6G -Xloggc:/data/deploy/tomcat_gc.log -XX:+UseParallelGC -XX:+PrintGCDetails -Dspring.profiles.active=dev"
...
stop() {
   ...
   if [ $psid -ne 0 ]; then
      echo -n "Stopping $APP_HOME $APP_MAINCLASS ...(pid=$psid) "
...
status() {
   ...
   if [ $psid -ne 0 ];  then
      echo "$APP_HOME $APP_MAINCLASS is running! (pid=$psid)"
   else
      echo "$APP_HOME $APP_MAINCLASS is not running"
   fi
}
